#!/bin/bash
#SBATCH --job-name=lenia_hpo_vit_v2
#SBATCH --output=logs_lenia_hpo_vit_v2_%j.out
#SBATCH --error=logs_lenia_hpo_vit_v2_%j.err
...
RESULTS_DIR="$HOME/lenia-results/hpo_vit_v2_$SLURM_JOB_ID"
...
echo "=== STEP 5: Run Pixel-ViT HPO ==="
export WANDB_MODE=offline
mkdir -p "$WORK_DIR/experiments"

$PYTHON scripts/run_hpo.py \
    --setup pixel \
    --config config_cluster_vit_v2.yaml \
    --storage "sqlite:///$PROJECT_DIR/experiments/hpo_vit_v2.db" \
    --study-name vit_v2_hpo
EXIT_CODE=$?
