#!/bin/bash
#SBATCH --job-name=lenia_hpo_pjepa_v2
#SBATCH --output=logs_lenia_hpo_patch_jepa_v2_%j.out
#SBATCH --error=logs_lenia_hpo_patch_jepa_v2_%j.err
...
RESULTS_DIR="$HOME/lenia-results/hpo_patch_jepa_v2_$SLURM_JOB_ID"
...
echo "=== STEP 5: Run Patch-JEPA HPO ==="
export WANDB_MODE=offline
mkdir -p "$WORK_DIR/experiments"

$PYTHON scripts/run_hpo.py \
    --setup patch_jepa \
    --config config_cluster_patch_jepa_v2.yaml \
    --storage "sqlite:///$PROJECT_DIR/experiments/hpo_patch_jepa_v2.db" \
    --study-name patch_jepa_v2_hpo
EXIT_CODE=$?
