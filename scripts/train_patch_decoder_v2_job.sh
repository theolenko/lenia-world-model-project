#!/bin/bash
#SBATCH --job-name=lenia_patch_dec_v2
#SBATCH --output=logs_lenia_patch_decoder_v2_%j.out
#SBATCH --error=logs_lenia_patch_decoder_v2_%j.err
...
RESULTS_DIR="$HOME/lenia-results/patch_decoder_v2_$SLURM_JOB_ID"
PYTHON="$HOME/.conda/envs/lenia-wm/bin/python"

JEPA_CKPT="$HOME/lenia-results/patch_jepa_v2_24523549/experiments/patch_jepa_20260625_163647/checkpoint_best.pt"
...
echo "=== STEP 5: Train Patch-JEPA decoder ==="
export WANDB_MODE=offline
mkdir -p "$WORK_DIR/experiments"

$PYTHON scripts/train_patch_decoder.py \
    --jepa-checkpoint "$JEPA_CKPT" \
    --config config_cluster_patch_decoder_v2.yaml
EXIT_CODE=$?
