#!/bin/bash
#SBATCH --job-name=lenia_jepa_training
#SBATCH --output=logs_lenia_jepa_%j.out
#SBATCH --error=logs_lenia_jepa_%j.err
...
RESULTS_DIR="$HOME/lenia-results/jepa_$SLURM_JOB_ID"
...
echo "=== STEP 6: Start JEPA training ==="
$PYTHON scripts/train_single.py --setup jepa --config config_cluster_jepa.yaml
EXIT_CODE=$?
...
if [ $EXIT_CODE -ne 0 ]; then
    echo "ERROR: JEPA training failed with exit code $EXIT_CODE"
    exit $EXIT_CODE
fi
